...
try:
    heatmap_avg = torch.zeros((len(multiplier), 19, oriImg.shape[0], oriImg.shape[1])).to(device)
    paf_avg = torch.zeros((len(multiplier), 38, oriImg.shape[0], oriImg.shape[1])).to(device)
except:
...
for m in range(len(multiplier)):
    ...
    output1, output2 = model.forward(feed)
    print(output1.size())
    print(output2.size())
...
for part in range(18):
    ...
    peaks_binary = np.logical_and.reduce(
        (map >= map_left, map >= map_right, map >= map_up, map >= map_down, map > param_['thre1']))
    #    peaks_binary = T.eq(
